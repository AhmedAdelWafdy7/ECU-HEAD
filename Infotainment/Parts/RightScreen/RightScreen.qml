...
Page {
    ...
    // Add a function to replace the missing adaptivewidth function
    function adaptivewidth(size) {
        return size * (width / 1280) // Base on standard 1280 width
    }

    // Add a default systemHandler to prevent null reference errors
    property var systemHandler: QtObject {
        property bool carLocked: false
        property string currentTime: "April 21, 2025"
        property string outdoorTemp: "72"
        property string username: "Driver"

        function setCarLocked(locked) {
            carLocked = locked
        }
    }
